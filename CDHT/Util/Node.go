...
// # ---------------------------- Init --------------------------------- # 
func (node *Node) initNode(port string) {
    node.Port = port
...
func (node *Node) CreateRing(port string) {
    node.initNode(port)
...
func (node *Node) Join(available *NodeRPC, port string) {
    node.initNode(port)
...
func (node *Node) FindSuccessor(nodeId *big.Int, remoteNode *NodeRPC) error {
    ...
    if between(node.Node_id, nodeId, succ.Node_id) || nodeId.Cmp(succ.Node_id) == 0 {
        ...
        err, pred := pred.FindSuccessor(nodeId)
        node.successor = &NodeRPC{}

        if err != nil || checkNode( pred ) == nil {
            node.GetNodeInfo(node.defaultArgs, remoteNode)
            return nil
        }

        copyNodeData(pred, remoteNode)
    }
    return nil
}
...
func  (node *Node)  closestPrecedingNode(nodeId *big.Int) *NodeRPC {
    ...
    for i := len(node.fingerTableEntry) - 1; i >= 0; i-- {
        ...
        if between(node.Node_id, entry.Node_id, nodeId){
            return entry
        }
    }
